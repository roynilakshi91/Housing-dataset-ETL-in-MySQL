...
1. Viewing the data using NILAKSHIDB database

SELECT * FROM NILAKSHIDB.housingdataseforetl



3. Standardize Sales Date format
...
4. Populate Property Address. Doing self join on  NILAKSHIDB.`HousingDataForETL`
...
5. Separating the Address into adress, city, state
...
6. Doing the same with Owner Address
...
7. Change Y and N to Yes and No to "Sold as Vacant" field. Finding the count and unique label of yes and no in "sold as vacant" field
...
8. Remove the duplicates. Finding duplicate rows using CTE (Common Table Expression) and Row_Number
